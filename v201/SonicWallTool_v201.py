VERSION = "2.0.1"  # Semantic Versioning: Update this for every new release
...
# Normalize Excel columns to match expected names
def normalize_excel_columns(df, expected_columns):
    """
    Normalize Excel column names so matching is:
    - Case-insensitive
    ...
    The rest of the program can continue using the
    canonical column names.
    """

    # Map normalized names to the canonical names used by the program
    expected_map = {
    ...
    for actual_column in df.columns:
        normalized = str(actual_column).strip().casefold()

        if normalized in expected_map:
            canonical = expected_map[normalized]

            # Detect duplicate columns such as:
            # Name + name
            if canonical in matched_columns:
                raise ValueError(
                    f"Duplicate Excel columns detected: "
                    f"'{matched_columns[canonical]}' and '{actual_column}' "
                    f"both match '{canonical}'"
                )

            matched_columns[canonical] = actual_column
            rename_map[actual_column] = canonical
...
class SonicToolApp:
    def __init__(self, root):
        ...
        self.tabs.add(self.tab2, text="User Group Generator")
        self.tabs.add(self.tab3, text="Address Object Generator")
        self.tabs.add(self.tab4, text="Service Object Generator")
        self.tabs.add(self.tab5, text="DHCP Static Generator")
        self.tabs.pack(expand=1, fill="both")
        self.build_user_tab(self.tab1)
        self.build_group_tab(self.tab2)
        self.build_address_tab(self.tab3)
        self.build_service_tab(self.tab4)
    ...
    def run_user_script(self):
        ...
        for input_path in self.user_files:
            ...
            try:
                ...
                df = normalize_excel_columns(df, ["Username", "Password", "Group"])
                ...
                out_fname = f"{fname}_Script_{ts_file}.txt"
    ...
    def run_address_script(self):
        ...
        for input_path in self.addr_files:
            ...
            try:
                df = pd.read_excel(input_path)
                in_fname = os.path.basename(input_path)
                df = normalize_excel_columns(df,["Name", "Address", "Zone", "Group"])
    ...
    def run_service_script(self):
        ...
        for input_path in self.svc_files:
            ...
            try:
                df = pd.read_excel(input_path)
                in_fname = os.path.basename(input_path)
                df = normalize_excel_columns(df, ["Name", "Protocol", "Port", "Group"])
    ...
    def run_group_script(self):
        ...
        for input_path in self.grp_files:
            ...
            try:
                df = pd.read_excel(input_path)
                in_fname = os.path.basename(input_path)
                df = normalize_excel_columns(df, ["Group", "Member"])
    ...
    def run_dhcp_script(self):
        ...
        for input_path in self.dhcp_files:
            ...
            try:
                ...
                df = normalize_excel_columns(df, ["Name", "MAC", "IP", "Mask", "GW", "DNS"])

                ts_file = datetime.now().strftime("%Y%m%d_%H%M%S")
                ts_log = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                fname = os.path.splitext(os.path.basename(input_path))[0]

                out_fname = os.path.join(output_folder, f"{fname}_Script_{ts_file}.txt")
